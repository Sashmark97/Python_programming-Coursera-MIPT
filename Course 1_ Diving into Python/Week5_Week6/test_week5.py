...
class ServerSocket:
    ...
    def send(self, data):
        if data in self.rsp_map:
            ...
            raise ServerSocketException("запрос не соответствует протоколу:")# {data}")
...
class TestClient(unittest.TestCase):
    ...
    def test_client_put(self):
        ...
        for metric, value, timestamp in metrics_for_put:
            try:
                ...
                message = exp.args[0]
                self.fail("Ошибка вызова client.put("
                          "'{metric}', {value}, timestamp={timestamp})\n{message}")
    ...
    def test_client_get_key(self):
        try:
            ...
            message = exp.args[0]
            self.fail("Ошибка вызова client.get('test')\n{message}")
    ...
    def test_client_get_all(self):
        try:
            ...
            message = exp.args[0]
            self.fail("Ошибка вызова client.get('*')\n{message}")
    ...
    def test_client_get_not_exists(self):
        try:
            ...
            message = exp.args[0]
            self.fail("Ошибка вызова client.get('key_not_exists')\n{message}")
    ...
    def test_client_get_client_error(self):
        try:
            ...
            message = exp.args[0]
            self.fail("Некорректно обработано сообщение сервера об ошибке: {message}")
