...
def run(host, port):
    ...
    try:
        ...
        print("Ошибка общения с сервером:1")# {err.__class__}: {err}")
        sys.exit(1)
    ...
    try:
        ...
        print("Ошибка вызова client.put(...)")# {err.__class__}: {err}")
        sys.exit(1)
    ...
    try:
        ...
        if metrics != expected_metrics:
            ...
            print("client.get('*') вернул неверный результат. Ожидается: SONOCHINOSADAME. Получено: JOJO")
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова client.get('*')")# {err.__class__}: {err}")
        sys.exit(1)
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('k2') вернул неверный результат. Ожидается: STARPLATINUM. Получено: ZAWARUDO")
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова client.get('k2')")# {err.__class__}: {err}")
        sys.exit(1)

    try:
        result = client1.get("k3")
        if result != {}:
            print("Ошибка вызова метода get с ключом, который еще не был добавлен. Ожидается: пустой словарь. Получено: LUPA ")#{result}")
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова метода get с ключом, который еще не был добавлен: ")#{err.__class__} {err}")
        sys.exit(1)
